...
### Added
- **Corrections & retractions (WS4):** editors can append a **public, dated correction** to a published article (`FR-EDIT-5`) — a trust surface. A new append-only `article_correction` table + migration; an editor-only `POST /v1/cms/articles/:id/corrections` that **strips markup** from the note and only applies to **published** articles; the read API serves an article's `corrections` (oldest first); the article page shows a clearly-styled **Correction(s)** notice with dates; and editors get an "Add a correction" form on the published-story page. Covered by unit tests (add/strip/empty/non-published + serving); the HTTP round-trip is pending local DB availability (Docker was down).
- **Media picker in the editor (WS3 complete):** a **"Choose from library"** picker (a modal grid of uploaded images, fetched via a server action) is now wired into the **block editor** — on image blocks and as **add-from-library** on gallery blocks — **and** into the **featured-image** field of the story form, which is now controlled so a pick fills its URL/alt/credit. Authors no longer paste URLs; they upload once and reuse. This closes **WS3 — the media library** (S3 driver + WebP/AVIF variants remain the prod-storage swap behind the same contract).
- **Media library UI (WS3, web):** a **`/dashboard/media`** page (linked from the newsroom nav) where staff **upload images** and **browse the library** as a thumbnail grid. Uploads go through a **BFF server action** that attaches the session's access token and forwards the multipart form to the API — the browser never sees the token (`05` §3.2). Includes alt/credit fields, an accepted-types/size hint, loading + error + success states, and an empty state.
